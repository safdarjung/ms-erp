...
## 11. Pricing memory, messages, briefing, email reading, share links (built 2026-09-16)

**Pricing memory.** `apps/web/src/lib/price-history.ts` reads past quotation and bill lines (tenant-scoped, cancelled bills excluded) by item words + optional customer. Two consumers: the line editor's autocomplete (`components/item-suggest.tsx` + `GET /api/items/suggest` — one tap fills description, last rate, HSN, unit and GST; the rate is only filled when the row's rate is empty, and Enter never replaces a freshly typed item unless the user arrowed into the list) and the assistant's instant `price_history` tool (returns lines + latest/min/median/max; the table is shown to the user). The system prompt now requires `price_history` before proposing rates and forbids inventing a "last time" figure.

**Messages (never sent by the app).** `draft_message` is an instant tool: the model writes the text (purpose, recipient by customer/lead uuid or bare phone, optional quotation/bill), `lib/agent-instant.ts` resolves the phone/email, inserts a **public share link** at `{{pdf_link}}`, appends the business sign-off, and the panel renders a *message card* — editable text, **Send on WhatsApp** (wa.me), **Send by email** (mailto), **Copy**. Entry points: assistant, "Ask AI" chips on quotation/bill/customer/enquiry pages, the dashboard's "Draft payment reminders" chip, and "what next" chips under a saved card (e.g. *Send quotation QT/… on WhatsApp*).

**Share links.** `lib/share.ts` signs `kind + id + tenant + expiry` with HMAC-SHA256 (derived from `SESSION_SECRET`); `/s/q/<id>/<token>` and `/s/b/<id>/<token>` render the same letterhead page the shop prints (`pdf/build-document.ts` is shared with the print routes) with a "Save as PDF" bar, `noindex`, no session. Cancelled bills and expired/tampered tokens 404. The detail pages' "Share on WhatsApp" now carries this link. Revocation = rotate `SESSION_SECRET`.

**Today's briefing.** `lib/briefing.ts` shapes `dashboardData()` into `BriefingFacts` (overdue with days late, due-soon, follow-ups, quotations sent 5+ days with no answer, orders due within 7 days, month billed/collected, receivables, week's enquiries/quotes, open orders); `packages/ai/src/briefing.ts` turns them into 3–5 bullet lines (≤90 words, numbers copied verbatim). Cached per tenant per IST day in `ai_briefing` (migration 0009, RLS'd); loaded client-side after paint (`dashboard/briefing-card.tsx`) so the dashboard never waits on the model; "Refresh" rewrites it. Fails soft.

**Email reading.** `packages/ai/src/extract-lead.ts` (structured output: isEnquiry, confidence, name/contact/phone/email/requirement/value/language). The ingest pipeline calls it **outside the DB transaction** only for mail that is neither a recognised marketplace layout nor bulk mail; confident non-enquiries (≥0.8) are parked as *Dismissed* (recoverable), otherwise the draft lands in the inbox as `parse_method='ai'` with its confidence and a "picked out by AI — check before creating" note. Nothing is auto-created from an AI read.

**Assistant robustness.** Both provider loops now share one `dispatchTool()`; a hallucinated tool name gets a corrective error naming the real tools; transient provider errors (5xx/unavailable/network) get one quiet retry; a turn that exhausts its tool rounds without writing anything ends with a plain "I got stuck…" line instead of silence; the prompt forbids re-running an identical query. Permissions are re-checked at **execute** time as well as at proposal time.

**AI activity page.** `/settings/ai` (settings.manage): requests/tokens/estimated cost this month and per day, by feature, by person, model mix, and every `ai_action` with its outcome (Saved / Not saved / Couldn't save / Waiting / Never answered).
